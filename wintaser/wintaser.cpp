 ...
 */
void DetectWindowsVersion(int& outMajor, int& outMinor)
{
    debugprintf("Detecting Windows version...\n");

    if (IsWindows8Point1OrGreater()) { outMajor = 6; outMinor = 3; }
    else if (IsWindows8OrGreater()) { outMajor = 6; outMinor = 2; }
    else if (IsWindows7OrGreater()) { outMajor = 6; outMinor = 1; }
    else if (IsWindowsVistaOrGreater()) { outMajor = 6; outMinor = 0; }
    else if (IsWindowsXPOrGreater()) { outMajor = 5; outMinor = 2; }
    else { outMajor = 1; outMinor = 0; }
...
HWND hWnd = 0;
//HWND hExternalWnd = 0;
HWND RamSearchHWnd = nullptr; // modeless dialog
HWND RamWatchHWnd = nullptr; // modeless dialog
HWND HotkeyHWnd = nullptr;
bool HotkeyHWndIsHotkeys = true;
//HWND SpliceHWnd = nullptr; // modeless dialog
...
char* NormalizePath(char* output, const char* path)
{
    ...
    if (/*!(movie.version >= 40 && movie.version < 53) || */!started)
        len = GetFullPathNameA(path, MAX_PATH, output, nullptr);
    if (len && len < MAX_PATH)
...
bool GetFileNameFromFileHandle(HANDLE hFile, TCHAR* pszFilename)
{
    ...
    bool bSuccess = false;
    if (HANDLE hFileMap = CreateFileMapping(hFile, nullptr, PAGE_READONLY, 0, 1, nullptr))
    {
        ...
        // load all the threads
        {
            ...
            for (unsigned int i = 0; i < state.threads.size(); i++)
            {
                ...
                if (hGameThreads[thread.id] != thread.handle)
                {
                    ...
                    //		nullptr,
                    //		0,
                    //		(LPTHREAD_START_ROUTINE)nullptr, // is this allowed?
                    //		nullptr,
                    //		nullptr,
                    //		&thread.id
                    //);
...
void RecordLocalInputs()
{
    ...
    {
        inputC.ProcessInputs(&frame.inputs, nullptr);
        //Update_Input(nullptr, true, false, false, false);
    }
    else
    {
        frame.inputs.clear();
...
void CheckDialogChanges(int frameCount)
{
        ...
        finished != displayed_finished)
    {
        ...
        InvalidateRect(GetDlgItem(hWnd, IDC_STATIC_FRAMESLASH), nullptr, TRUE);
...
void SleepFrameBoundary(const char* frameInfo, int threadId)
{
    ...
    if (localTASflags.aviMode & 1)
    {
        ProcessCaptureFrameInfo(nullptr, CAPTUREINFO::TYPE_PREV);
    }
...
void FrameBoundary(const char* frameInfo, int threadId)
{
    ...
    CAPTUREINFO frameCaptureInfoType;
    ...
    if (!requestedCommandReenter)
    {
        ...
        if (frameCaptureInfoType != CAPTUREINFO::TYPE_NONE_SUBSEQUENT)
        {
        ...
        temporaryUnpause = (frameCaptureInfoType == CAPTUREINFO::TYPE_PREV) && advancePastNonVideoFrames;
    }
    else
    {
...
void PrintPrivileges(HANDLE hProcess)
{
    ...
    {
        DWORD length = 0;
        GetTokenInformation(htok, TokenDefaultDacl, nullptr, 0, &length);
...
{
    SearchPath(nullptr, buf, nullptr, MAX_PATH, buf, nullptr);
...
static HANDLE debuggerThread = 0;
...
bool AdjustPrivileges(HANDLE hGameHandle, bool bEnable)
{
    ...
    BOOL lookedUp = LookupPrivilegeValue(nullptr, SE_DEBUG_NAME, &luid);
    if (lookedUp == FALSE)
    {
...
//		default: {
//			const char* desc = ExceptionCodeToDescription(exitcode, nullptr);
//			if(desc)
...
static DWORD WINAPI DebuggerThreadFunc(LPVOID lpParam)
{
    ...
    BOOL created = CreateProcess(exefilename, // application name
        ...
        nullptr, // process attributes (e.g. security descriptor)
        nullptr, // thread attributes (e.g. security descriptor)
    ...
    if (limitGameToOnePhysicalCore) // We want to limit the game to one physical core to avoid issues
    {
        ...
        CPUInfo(&logicalCores, &physicalCores, /*&hyperThreading*/ nullptr);
    ...
    {
        EXTENDEDTRACEINITIALIZEEX(nullptr, hGameProcess);
    }
...
#pragma region UNLOAD_DLL_DEBUG_EVENT
                    ...
                    //dllBaseToHandle[de.u.LoadDll.lpBaseOfDll] = nullptr;
...
#pragma region CREATE_PROCESS_DEBUG_EVENT
                    ...
                    if (nullFile)
                        de.u.CreateProcessInfo.hFile = CreateFile(filename, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
                    ...
                    {	// TODO: WHY?
                        ...
                        de.u.CreateProcessInfo.hFile = nullptr;
                    }
#pragma endregion
                }
                CloseHandle(de.u.CreateProcessInfo.hFile);
...
HANDLE hAfterDebugThreadExitThread = nullptr;
...
static DWORD WINAPI AfterDebugThreadExitThread(LPVOID lpParam)
{
    ...
    hAfterDebugThreadExitThread = nullptr;
...
void OnAfterDebugThreadExit()
{
    ...
    hAfterDebugThreadExitThread = CreateThread(nullptr, 0, AfterDebugThreadExitThread, nullptr, 0, nullptr);
}
...
rewait:
    ...
    while (debuggerThread && ((timeGetTime() - prevTime) < maxWaitTime))
    {
        ...
        MSG msg;
        int count = 0;
        while (count < 4 && PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))
        {
            DispatchMessage(&msg);
            count++;
        }
    ...
    int       nCmdShow)
{
    ...
    while (true)
    {
        ...
        //DWORD ret = MsgWaitForMultipleObjects(0, nullptr, FALSE, started ? 1000 : 30, QS_ALLINPUT);
        ...
        else
        {
            ...
            MsgWaitForMultipleObjects(0, nullptr, FALSE, started ? 1000 : 30, QS_ALLINPUT);
        }

        //if(!debuggerThread && (GetAsyncKeyState('G') & 0x8000))
        //{
        //	debuggerThread = CreateThread(nullptr, 0, DebuggerThreadFunc, nullptr, 0, nullptr);
        //}
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    hWnd = CreateDialog(hInstance, MAKEINTRESOURCE(IDD_DIALOG1), nullptr, (DLGPROC)DlgProc);
    ...
    //   SetWindowPos(hExternalWnd, nullptr, 0,340, 0, 0, SWP_NOSIZE|SWP_NOZORDER);
...
static void EnableDisablePlayRecordButtons(HWND hDlg)
{
    ...
    bool exists = true;
    bool exeexists = true;
    FILE* moviefile = *moviefilename ? fopen(moviefilename, "r+b") : nullptr;
    if (moviefile)
        fclose(moviefile);
    else
    {
        moviefile = *moviefilename ? fopen(moviefilename, "rb") : nullptr;
        if (moviefile)
            fclose(moviefile);
        else
            exists = false;

        moviefile = *moviefilename ? fopen(moviefilename, "ab") : nullptr;
        if (moviefile)
        {
            fclose(moviefile);
            if (!exists)
    ...
    FILE* exefile = *exefilename ? fopen(exefilename, "rb") : nullptr;
    if (exefile)
        fclose(exefile);
    else
        exeexists = false;
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        {
            ...
            case ID_SWITCH_TO_TASEE_FROM_TASER:
                ...
                SetWindowPos(HotkeyHWnd, nullptr, rect.left, rect.top, rect.right-rect.left, rect.bottom-rect.top, SWP_NOSIZE|SWP_NOZORDER);
            ...
            case IDC_TEXT_EXE:
                if (HIWORD(wParam) == EN_CHANGE || !movieFileExists || !*moviefilename)
                {
                    ...
                    //FILE* file = *exefilename ? fopen(exefilename, "rb") : nullptr;
                    //if(file)
                    //{
                    //	fclose(file);
    ...
    case WM_DROPFILES:
        ...
        {
            HDROP hDrop = (HDROP)wParam;
            int count = (int)DragQueryFile(hDrop, (UINT)-1, nullptr, 0);
            for (int i = 0; i < count; i++)
            {
                char filename[MAX_PATH + 1];
